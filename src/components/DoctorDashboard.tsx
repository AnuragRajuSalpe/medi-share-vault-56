...
import { QrCode, Stethoscope, Users, Clock, Download, TrendingUp } from 'lucide-react';
...
export default function DoctorDashboard({ user, accessLogs, onLogAccess }: DoctorDashboardProps) {
  ...
  useEffect(() => {
    if (user) {
      fetchDoctorData();
      const cleanup = subscribeToQRCodes();
      return cleanup;
    }
  }, [user, fetchDoctorData, subscribeToQRCodes]);
  ...
  const subscribeToQRCodes = useCallback(() => {
  ...
  const handleAccessQR = async (qrCode: PatientQRCode) => {
    try {
      ...
      // Call the onLogAccess callback to update the access logs
      if (qrCode.record_ids.length > 0) {
        onLogAccess(qrCode.record_ids[0]);
      }
                  ...
                  <div className="space-y-2 max-h-32 overflow-y-auto">
                    {patientQRCodes.slice(0, 2).map((qrCode) => (
                      ...
                      >
                        ...
                        <Badge variant={isExpired(qrCode.expires_at) ? 'destructive' : 'default'} className="text-xs">
                          {isExpired(qrCode.expires_at) ? 'Expired' : 'Active'}
                        </Badge>
                      </div>
                    ))}
                    {patientQRCodes.length > 2 && (
                      <p className="text-xs text-muted-foreground">
                        +{patientQRCodes.length - 2} more
                      </p>
                    )}
                  </div>
                ) : (
        ...
        <CardContent>
          {recentLogs.length > 0 ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-muted-foreground">
                Access patient QR codes to see access history here
              </p>
            </div>
          )}
        </CardContent>
